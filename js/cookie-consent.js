(function() {
  ...
  var prior = localStorage.getItem('cookie_consent');

  // Returning visitor who accepted: restore Consent Mode v2 grant immediately, no DOM work.
  if (prior === 'accepted') {
    if (typeof gtag === 'function') gtag('consent', 'update', GRANTED);
    return;
  }
  // Returning visitor who rejected: defaults stay denied; no banner.
  if (prior) return;
